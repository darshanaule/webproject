  ...
  Adjust the path to start from the bottom-left corner and increase the diameter of the semicircle 
       <path d="M0,100 Q34,15 260,4" stroke="black" stroke-width="1" fill="none" stroke-dasharray="5,5" opacity="0.7"/>
        ...
        <div class="character character1">
            <img src="assets/images/jet.png" class="projectile-image-1">
        </div>
...
<section class="our_services">
  ...
  Adjust the path to start from the bottom-left corner and increase the diameter of the semicircle 
       <path d="M0,100 Q34,15 260,4" stroke="black" stroke-width="1" fill="none" stroke-dasharray="5,5" opacity="0.7"/>
        ...
        <!-- Image with semicircular motion animation -->
        <div class="character character1">
            <img src="assets/images/jet.png" class="projectile-image-2">
        </div>
<div class=" plans_container" >
<div class="plans_card">
      <!-- Card content here -->
...
<script>
$(document).ready(function(){
    var laptop1AnimationsTriggered = false;
    var laptop2AnimationsTriggered = false;

    $(window).scroll(function() {
        if (!laptop1AnimationsTriggered) {
            var windowScroll = $(window).scrollTop();
            var laptop1Offset = $('.laptop_1').offset().top;
            var windowHeight = $(window).height();

            // Check if the user has scrolled to the laptop_1 div
            if (windowScroll > (laptop1Offset - windowHeight / 2)) {
                // Trigger the jet animation for laptop_1
                $('.projectile-image-1').addClass('projectile-animation-1');

                // Trigger the card animations one by one for laptop_1
                $('.laptop_1 .plans_card').each(function(index) {
                    var card = $(this);
                    setTimeout(function() {
                        card.css('opacity', '1').addClass('slideUp');
                    }, index * 500); // Adjust the delay between each card appearance
                });

                laptop1AnimationsTriggered = true; // Set flag to true to prevent multiple triggers
            }
        }

        if (!laptop2AnimationsTriggered) {
            var windowScroll = $(window).scrollTop();
            var laptop2Offset = $('.laptop_2').offset().top;
            var windowHeight = $(window).height();

            // Check if the user has scrolled to the laptop_2 div
            if (windowScroll > (laptop2Offset - windowHeight / 2)) {
                // Trigger the jet animation for laptop_2
                $('.projectile-image-2').addClass('projectile-animation-2');

                // Trigger the card animations one by one for laptop_2
                $('.laptop_2 .plans_card').each(function(index) {
                    var card = $(this);
                    setTimeout(function() {
                        card.css('opacity', '1').addClass('slideUp');
                    }, index * 500); // Adjust the delay between each card appearance
                ...
                laptop2AnimationsTriggered = true; // Set flag to true to prevent multiple triggers
            }
        }
    });
});
...
$(document).ready(function(){
    ...
    $(window).scroll(function() {
        if (!laptop1AnimationsTriggered) {
            var windowScroll = $(window).scrollTop();
            var laptop1Offset = $('.mobile_1').offset().top;
            var windowHeight = $(window).height();
            ...
            if (windowScroll > (laptop1Offset - windowHeight / 2)) {
                ...
                // Trigger the card animations one by one for laptop_1
                $('.mobile_1 .plans_card').each(function(index) {
                    var card = $(this);
                    setTimeout(function() {
                        card.css('opacity', '1').addClass('slideUp');
                    }, index * 500); // Adjust the delay between each card appearance
        ...
        if (!laptop2AnimationsTriggered) {
            var windowScroll = $(window).scrollTop();
            var laptop2Offset = $('.mobile_2').offset().top;
            var windowHeight = $(window).height();
            ...
            if (windowScroll > (laptop2Offset - windowHeight / 2)) {
                ...
                // Trigger the card animations one by one for laptop_2
                $('.mobile_2 .plans_card').each(function(index) {
                    var card = $(this);
                    setTimeout(function() {
                        card.css('opacity', '1').addClass('slideUp');
                    }, index * 500); // Adjust the delay between each card appearance
...
.projectile-animation-1 {
   /*  animation: semicircularMotion 5s linear forwards; Adjust the duration as needed */
    animation: semicircularMotion 5s linear forwards;
}
